...
void CRCNRStructure::FillGlobalMatrix(complex<double> **Ym, double w)
{
  ...
  complex<double> Y = (p*m_C)/((double)(4*m_x*m_y));
...
void CRCNRStructure::Dorabotka()
{
  ...
  {
    if (m_pMasKP[0][i].KPType==KPNORMAL)
    {
      pElement = &m_pMatrix[m_pMasKP[0][i].x][m_pMasKP[0][i].y];
      *pElement |= 1;
    }
    if (m_pMasKP[1][i].KPType==KPNORMAL)
    {
...
int CRCNRStructure::FixElement(int Element)
{
